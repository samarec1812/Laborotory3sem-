...
vector<string> FindNames(const map<int, string>& names_by_year,
                                int year) {
    vector<string> names;
...
public:
    ...
    string GetFullNameWithHistory(int year) {
        ...
        const string first_name = BuildName(FindFirstNamesHistory(year));
        // получим полную фамилию со всей историей
        const string last_name = BuildName(FindLastNamesHistory(year));
...
private:
    vector<string> FindFirstNamesHistory(int year) {
        return FindNames(first_names, year);
    }
    vector<string> FindLastNamesHistory(int year) {
        return FindNames(last_names, year);
    }
